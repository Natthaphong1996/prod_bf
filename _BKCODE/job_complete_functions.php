...
function insertJobComplete(
    ...
    // 1) เตรียม SQL สำหรับ INSERT  
    $sql = "
        INSERT INTO jobs_complete 
            (job_id, prod_complete_qty, receive_by, send_by, assembly_point, reason, date_complete, record_by)
        VALUES
            (?, ?, ?, ?, ?, ?, ?, ?)
    ";

    // 2) เตรียม statement และ bind parameter เพื่อป้องกัน SQL Injection  
    $stmt = $conn->prepare($sql);
    $stmt->bind_param(
        "sissssss",
        $job_id,            // string
        $prod_complete_qty, // integer
        $receive_by,        // string
        $send_by,           // string
        $assembly_point,    // string
        $reason,            // string
        $date_complete,     // string (datetime)
        $record_by          // string
    );

    // 3) Execute และคืนค่า true/false  
    return $stmt->execute();
}
