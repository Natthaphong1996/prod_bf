...
// ฟังก์ชันสำหรับดึงข้อมูล production_wages และนับจำนวนข้อมูล รวมถึงสร้าง Invoice

/**
 * ดึงข้อมูลพร้อมกรอง search และ date range
 *
 * @param mysqli $conn
 * @param int    $current_page
 * @param int    $items_per_page
 * @param string $search_value
 * @param string $date_from      // รูปแบบ YYYY-MM-DD
 * @param string $date_to        // รูปแบบ YYYY-MM-DD
...
function getProductionWagesList(
    ...
    $query = "
        ...
        JOIN jobs_complete jc 
          ON jc.job_id = SUBSTRING_INDEX(
                          SUBSTRING_INDEX(
                            SUBSTRING_INDEX(pw.job_id, '}', 1), '{', -1
                          ), ',', 1
                       )
        JOIN wood_issue wi 
          ON wi.job_id = jc.job_id
        JOIN product_price pp 
          ON pp.prod_id = wi.prod_id
        WHERE 1=1
          {$search_condition}
          {$date_condition}
        ORDER BY pw.id DESC
...
/**
 * สร้าง Invoice โดยรวม job_ids ที่เลือกเป็นสตริงและคำนวณผลรวมของ production_wage_price จาก jobs_complete
 * จากแต่ละ job_id แล้ว INSERT ลงใน production_wages โดยใช้คอลัมน์ total_wage
 *
 * @param mysqli $conn การเชื่อมต่อฐานข้อมูล
 * @param array $job_ids_array รายการ job_id ที่เลือก
 * @return mixed คืนค่าสตริงของ job_ids ที่รวมกัน หากสำเร็จ หรือ false หากไม่สำเร็จ
 */
function createInvoice($conn, $job_ids_array)
{
    if (empty($job_ids_array)) {
        return false;
    }

    // ป้องกัน SQL Injection สำหรับแต่ละ job_id
    $job_ids_escaped = array_map(function ($val) use ($conn) {
        return mysqli_real_escape_string($conn, $val);
    }, $job_ids_array);

    // ตรวจสอบและกรอง job_id ที่ยังไม่ได้ invoiced (issue_status != 'สั่งจ่ายแล้ว')
    $job_ids_to_invoice = array();
    foreach ($job_ids_escaped as $jid) {
        $query = "SELECT issue_status FROM wood_issue WHERE job_id = '$jid' LIMIT 1";
        $result = mysqli_query($conn, $query);
        if ($result && mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_assoc($result);
            if ($row['issue_status'] !== 'สั่งจ่ายแล้ว') {
                $job_ids_to_invoice[] = $jid;
            }
        }
    }

    // หากไม่มี job_id ที่ยังไม่ได้ invoiced
    if (empty($job_ids_to_invoice)) {
        return false;
    }

    // สร้าง $job_ids_string ในรูปแบบ {job_id,price_value}
    $job_ids_string_array = array();
    foreach ($job_ids_to_invoice as $jid) {
        // ดึง prod_id จาก wood_issue
        $query_prod = "SELECT prod_id FROM wood_issue WHERE job_id = '$jid' LIMIT 1";
        ...
        // ดึง price_value จาก product_price โดยใช้ prod_id ที่ได้
        $query_price = "SELECT price_value FROM product_price WHERE prod_id = '$prod_id' LIMIT 1";
        $result_price = mysqli_query($conn, $query_price);
        if ($result_price && mysqli_num_rows($result_price) > 0) {
            $row_price = mysqli_fetch_assoc($result_price);
            $price_value = $row_price['price_value'];
        } else {
            $price_value = 0;
        }
        $tuple = "{" . $jid . "," . $price_value . "}";
        $job_ids_string_array[] = $tuple;
    }
    $job_ids_string = implode(',', $job_ids_string_array);

    // คำนวณผลรวมของ production_wage_price จากตาราง jobs_complete สำหรับ job_id ที่เลือก
    $total_wage = 0;
    foreach ($job_ids_to_invoice as $jid) {
        $query = "SELECT SUM(production_wage_price) AS total FROM jobs_complete WHERE job_id = '$jid'";
        $result = mysqli_query($conn, $query);
        if ($result && mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_assoc($result);
            $total_wage += (float)$row['total'];
        }
    }

    // กำหนดโซนเวลาเป็นประเทศไทย (Asia/Bangkok)
    date_default_timezone_set('Asia/Bangkok');
    // สร้าง production_wage_id ในรูปแบบ "PW-DDMMYYHHMMSSSS"
    $micro = sprintf("%02d", (int)((microtime(true) - floor(microtime(true))) * 100));
    $production_wage_id = "PW-" . date("dmy-His");

    // INSERT Invoice ลงในตาราง production_wages โดยใช้ production_wage_id, job_id (ในรูปแบบ tuple) และ total_wage
    $query = "INSERT INTO production_wages (production_wage_id, job_id, total_wage)
              VALUES ('$production_wage_id', '$job_ids_string', '$total_wage')";
    if (mysqli_query($conn, $query)) {
        // อัปเดต wood_issue เปลี่ยน issue_status เป็น 'สั่งจ่ายแล้ว' สำหรับ job_id ที่ invoiced
        $job_ids_for_update = implode("','", $job_ids_to_invoice);
        $update_query = "UPDATE wood_issue SET issue_status = 'รอยืนยันการสั่งจ่าย' WHERE job_id IN ('$job_ids_for_update')";
