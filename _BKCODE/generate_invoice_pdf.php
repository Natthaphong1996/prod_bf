...
foreach ($jobIdsArray as $item) {
    ...
    // jobs_complete
    $stmt3 = $conn->prepare("SELECT prod_complete_qty, assembly_point, date_complete FROM jobs_complete WHERE job_id = ?");
    $stmt3->bind_param("s", $job_id);
    $stmt3->execute();
    $stmt3->bind_result($qty, $point, $date_complete);
    $stmt3->fetch();
    $stmt3->close();
...
usort($jobs, function($a, $b) {
    ...
    $dateA = date('Y-m-d', strtotime($a['date_complete']));
        ...
        <table style="width: 100%; text-align: center;">
            <tr>
                <td><div style="border-bottom: 1px solid #000; width: 50%; margin: auto;"></div>ผู้ออกเอกสาร<br>' 
                    . htmlspecialchars($staffName) . '<br>เจ้าหน้าที่</td>
