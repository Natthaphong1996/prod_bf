...
$whereClauses[] = "wi.issue_status IN ('สั่งไม้', 'กำลังเตรียมไม้', 'รอเบิก', 'เบิกแล้ว')";
...
<body>
    ...
    <div class="container mt-5">
        ...
        <form method="GET" action="">
            ...
            <a href="nails_issue_list.php" class="btn btn-secondary">รีเซ็ต</a>
        </form>
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()) { ?>
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-warning btn-sm open-nail-modal"
                                data-issue-id="<?= $row['issue_id'] ?>" data-prod-id="<?= $row['prod_id'] ?>"
                                data-job-id="<?= $row['job_id'] ?>">
                                เบิกตะปู
                            </button>
                            <a href="generate_nail_pdf.php?issue_id=<?= $row['issue_id'] ?>" class="btn btn-info btn-sm"
                                target="_blank">PDF</a>
                        </td>
                    </tr>
                <?php } ?>
            ...
            <div class="modal-content">
                <form id="nailForm">
                    <div class="modal-header">
                        <h5 class="modal-title">เบิกตะปู</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <div id="nailListContainer">
                            <p class="text-muted">กำลังโหลดรายการตะปูจาก BOM...</p>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <input type="hidden" name="issue_id" id="modalIssueId">
                        <input type="hidden" name="job_id" id="modalJobId">
                        <button type="submit" class="btn btn-success">บันทึกการเบิก</button>
    ...
    <script>
        ...
        $(document).ready(function () {
            $('.open-nail-modal').click(function () {
                ...
                $('#nailListContainer').html('<p class="text-muted">กำลังโหลดข้อมูล...</p>');

                $.post('get_bom_nails.php', { prod_id: prodId, job_id: jobId }, function (response) {
                    $('#nailListContainer').html(response);
                });

                const modal = new bootstrap.Modal(document.getElementById('nailModal'));
                modal.show();
            });
            ...
            $('#nailForm').submit(function (e) {
                ...
                $.post('save_nail_usage.php', formData, function (response) {
                    const res = JSON.parse(response);
                    alert(res.message);
                    if (res.success) {
                        bootstrap.Modal.getInstance(document.getElementById('nailModal')).hide();
